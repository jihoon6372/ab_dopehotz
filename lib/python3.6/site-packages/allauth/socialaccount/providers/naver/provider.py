...
class NaverProvider(OAuth2Provider):
    ...
    def sociallogin_from_response(self, request, response):
        ...
        if not sociallogin.user.username:
            sociallogin.user.username = 'naver_%s' % sociallogin.account.extra_data.get('name', sociallogin.account.extra_data['nickname'])
        sociallogin.user.profile_picture = sociallogin.account.extra_data['profile_image']
        return sociallogin
